...
<form action="{{ route('backend.treatment.update', $treatment->id) }}" method="POST" style="max-width: 500px;">
    @csrf
    @method('PUT')
    ...
    <div class="mb-3">
        ...
        <select name="treatment_category_id" class="form-select" required>
            ...
            @foreach ($categories as $cat)
                <option value="{{ $cat->id }}"
                    {{ old('treatment_category_id', $treatment->treatment_category_id) == $cat->id ? 'selected' : '' }}>
                    {{ $cat->name }}
                </option>
            @endforeach
        </select>
    ...
    <div class="mb-3">
        ...
        <input type="text" name="name" class="form-control"
            value="{{ old('name', $treatment->name) }}" required maxlength="50">
    </div>
